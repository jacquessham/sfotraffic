...
# Partition df_2022 less than Jun 2022
y_plot = df_2022[df_2022['date']<='2022-06-30']

# Import prediction
df_pred = pd.read_csv('../Part2/Results/prediction_step3.csv')
# Shift date to end of the month
df_pred['date'] = pd.to_datetime(df_pred['date'], format='%Y-%m-%d')

# Calculate R2 score
y = df_2022[(df_2022['date']>= '2021-01-01')&(df_2022['date']<='2022-06-30')][['date','pax_count']]
y_hat = df_pred[(df_pred['date']>= '2021-01-01')&(df_pred['date']<='2022-06-30')][['date','prediction']]
y_hat['prediction'] = y_hat['prediction']/1000000
r2 = r2_score(y['pax_count'],y_hat['prediction'])

# Calculate MSE
mse_result = mse(y['pax_count'],y_hat['prediction'])
...
data.append(go.Scatter(x=y_hat['date'],y=y_hat['prediction'],
...
plotly.offline.plot(fig, filename='result_part2.html')
